...
const SavedInvoicesList = ({ setModalState }: SavedInvoicesListProps) => {
    ...
    const getAllInvoicesById = async () => {
        setLoading(true);
        try {
            const list: InvoiceTypeReturnDb[] = await getAllInvoicesFromIdDb(user?.id ?? "ß");
    ...
    const handleDownload = async (inv: InvoiceTypeReturnDb) => {
        const blob = await generatePdf(inv.data);
        return downloadPdf(blob as Blob);
    };
                        ...
                        <CardContent className="flex justify-between">
                            <div>
                                ...
                                <div className="mt-2">
                                    ...
                                    <p>
                                        Receiver: {invoice.data.receiver.name}
                                    </p>
