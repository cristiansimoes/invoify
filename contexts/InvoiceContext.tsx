...
type Ctx = {
    ...
    onFormSubmit: (values: InvoiceType, edit: boolean, id?: string) => void;
    newInvoice: () => void;
    ...
    saveInvoice: (isEditing: boolean, id: string) => void;
...
export const InvoiceContextProvider = ({
    ...
    const { saveInvoiceDb, updateInvoiceDb } = useSupabase();
    ...
    useEffect(() => {
        try {
            ...
            const fixed = list.map((inv) => {
                ...
                return {
                    ...inv,
                    total: Number.isFinite(newTotal)
                        ? newTotal
                        : Number(inv.total) || 0,
    ...
    const previewPdfInTab = () => {
        if (invoicePdf.size > 0)
            window.open(URL.createObjectURL(invoicePdf), "_blank");
    };
    ...
    const onFormSubmit = async (
        ...
        if (edit) {
            await updateInvoiceDb(data, id ?? "", userId); // editing on DB
        } else {
            await saveInvoiceDb(data, userId); // saving on DB
        }
        
        localStorage.setItem("invoice_count", String(count + 1));
        edit ? modifiedInvoiceSuccess() : saveInvoiceSuccess();
        const blob = await generatePdf(data);
        downloadPdf(blob);
    };

    /** Save (edição) — preserva status existente */
    const saveInvoice = async (isEditing?: boolean, invId?: string) => {
        ...
        const userId = user?.id ?? "";
        if (isEditing) {
            await updateInvoiceDb(data, invId ?? "", userId); // editing on DB
        } else {
            await saveInvoiceDb(data, userId); // saving on DB
        }
    ...
    const exportInvoiceAs = (type: ExportTypes) =>
        exportInvoice(type, getValues());
    ...
    const loadInvoiceById = (id: string | number) =>
        getInvoiceByIdLocal(String(id));
